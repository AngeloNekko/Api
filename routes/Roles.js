...
router.get("/", async function (req, res, next) {
  ...
  .post(`/`,DataValidatorRoles("body", Roles),  async (req, res) => {
    try {
      let { body: nombre_rol } = req;
      let { body: permisos_ids } = req;
      const result = await createRoles(nombre_rol, permisos_ids);
      res.status(200).json({
        msg: "Permiso Creado",
        body: result,
      });
  ...
  .put(`/:id`,DataValidatorRoles("body", Roles), async (req, res) => {
    try {
      ...
      let { body: nombre_rol} = req;
      let { body: permisos_ids} = req;
      const result = await UpdateRoles(id, nombre_rol, permisos_ids);
      res.status(200).json({
