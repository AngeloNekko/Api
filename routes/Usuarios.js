...
router.get(`/`,async function (req, res, next) {
  ...
  .post(`/` ,DataValidatorUsuario("body", nombreUsuario),async (req, res) => {
    try {
      ...
      const result = await createUsuario(primer_nombre, segundo_nombre, primer_apellido, segundo_apellido);
      res.status(200).json({
        msg: "Usuario creado",
        body: result,
      });
  ...
  .put(`/:id`,DataValidatorUsuario("body", nombreUsuario), async (req, res) => {
    try {
      ...
      const result = await UpdateUsuario(id, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido);
      res.status(200).json({
