...
const createUsuario = (primer_nombre, segundo_nombre, primer_apellido, segundo_apellido) =>
  new Promise(async (resolve, reject) => {
    try {
      const DB = await MongoConnection();
      const permi = DB.collection(COLLECTION);
  ...
  const UpdateUsuario = (id, primer_nombre, segundo_nombre, primer_apellido, segundo_apellido) =>
  new Promise(async (resolve, reject) => {
    try {
      const DB = await MongoConnection();
      const permi = DB.collection(COLLECTION);
      const result = await permi.UpdateOne(
        {"_id": ObjectId(id) },
        {
          $set: {
            primer_nombre,
